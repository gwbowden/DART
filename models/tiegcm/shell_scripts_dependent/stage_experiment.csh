...
echo TIEGCMDIR == \${TIEGCMDIR}
echo TGCMDATA == \${TGCMDATA}

setenv     DARTDIR \${BASE}/models/tiegcm
setenv  EXPERIMENT /scratch/n23/gwb112/swm_project/gold_test_large/initial
setenv ENSEMBLEDIR /scratch/n23/gwb112/swm_project/gold_test_large/initial
setenv      OBSDIR /scratch/n23/gwb112/swm_project/hourly_gold_obs/gold_obs_201830500_to_201831423
echo DARTDIR == \${DARTDIR}
echo EXPERIMENT == \${EXPERIMENT}
echo ENSEMBLEDIR == \${ENSEMBLEDIR}

setenv     GPINAME gpi_2018280-2018324.nc
setenv     IMFNAME imf_OMNI_2018280-2018325.nc
EndOfText
...
while ( $instance <= $NUM_ENS )
   ...
   set gpi_file    = $GPINAME.`printf "%04d"            $instance`
   set imf_file    = $IMFNAME.`printf "%04d"            $instance`

   ${COPY} ${ENSEMBLEDIR}/$tiesecond  tiegcm_s.nc         || exit 2
   ${COPY} ${ENSEMBLEDIR}/$tierestart tiegcm_restart_p.nc || exit 2
   ${COPY} ${ENSEMBLEDIR}/$gpi_file $GPINAME              || exit 2
   ${COPY} ${ENSEMBLEDIR}/$imf_file $IMFNAME              || exit 2

   # Ensure that the tiegcm.nml for all the ensemble members is identical
   # in all the ways that matter.

   set gpi = "GPI_NCFILE = '$GPINAME'"
   set imf = "IMF_NCFILE = '$IMFNAME'"
