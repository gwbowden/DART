...
#PBS -j oe
#PBS -r y
#PBS -v PBS_ARRAY_INDEX

#===============================================================================

#-------------------------------------------------------------------------------
# STEP 0: Get PBS environment variables
#-------------------------------------------------------------------------------

echo "STEP 0: Get PBS environment variables."
...
#-------------------------------------------------------------------------------
# STEP 2: Move to the directory specified for this instance, prepare.
#-------------------------------------------------------------------------------

echo "STEP 2: Move to the directory specified for this instance, prepare."
...
echo "STEP 3: Convert the DART output file to form needed by model."
...
if ( -e tiegcm.nml.updated ) then
   echo "tiegcm.nml updated with new start/stop time for ensemble member $instance" >> $logfile
   mv -v tiegcm.nml tiegcm.nml.original
   mv -v tiegcm.nml.updated tiegcm.nml
else
   echo "ERROR tiegcm.nml did not update correctly for ensemble member $instance." >> $logfile
   exit 2
endif
...
echo "STEP 5: Convert the model output to form needed by DART."

echo "Starting model_to_dart at "`date`  >>  $logfile
../model_to_dart                         >>& $logfile || exit 5
